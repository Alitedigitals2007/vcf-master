...
Process VCF files without needing the web interface
"""
...
def main():
    parser = argparse.ArgumentParser(description='Process VCF files - Terminal based version')
    parser.add_argument('files', nargs='+', help='VCF files to process')
    ...
    for file_path in args.files:
        ...
        if not file_path.lower().endswith('.vcf'):
            print(f"Warning: {file_path} does not have .vcf extension, skipping")
            continue
        vcf_files.append(file_path)
    
    if not vcf_files:
        print("Error: No valid VCF files provided")
        sys.exit(1)
    
    # Read file contents
    print(f"Reading {len(vcf_files)} VCF file(s)...")
    files_content = []
    ...
    try:
        ...
        for filename, content in outputs.items():
            ...
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"  [OK] Saved {filename} ({len(content)} characters)")
